...
if ( ! class_exists( 'Lunar_Phase_Stock_Widget' ) ) {
	...
	class Lunar_Phase_Stock_Widget {
		...
		public function register_admin_menu() {
			add_options_page(
				__( 'Celestial Lunar Phase', 'celestial-lunar-phase' ),
				__( 'Celestial Lunar Phase', 'celestial-lunar-phase' ),
				'manage_options',
				'lpsw-settings',
		...
		public function register_settings() {
			...
			add_settings_section(
				...
				function () {
					echo '<p>' . esc_html__( 'Enter your API key and a default location. You can also override the location in the shortcode or Gutenberg block.', 'celestial-lunar-phase' ) . '</p>';
				},
				'lpsw-settings'
			);

			$fields = array(
				'api_key'          => __( 'WeatherAPI Key', 'celestial-lunar-phase' ),
				'default_location' => __( 'Default Location', 'celestial-lunar-phase' ),
				...
				'show_credit'      => __( 'Show Data Credit', 'celestial-lunar-phase' ),
			);
			...
			switch ( $key ) {
				case 'api_key':
					printf(
						...
						esc_attr( $key ),
						esc_attr( $value ),
						esc_html__( 'Required. Get a key from WeatherAPI.com.', 'celestial-lunar-phase' )
					);
					break;
				...
				case 'default_location':
					printf(
						...
						esc_attr( $key ),
						esc_attr( $value ),
						esc_html__( 'Examples: "Winona, MN", "10001", or "44.05,-91.64".', 'celestial-lunar-phase' )
					);
					break;
				...
				case 'show_credit':
					printf(
						...
						esc_html__( 'Enabled', 'celestial-lunar-phase' )
					);
					break;
			}
		}
		...
		public function render_settings_page() {
			...
			?>
			<div class="wrap">
				<h1><?php esc_html_e( 'Celestial Lunar Phase', 'celestial-lunar-phase' ); ?></h1>
				...
				<h2><?php esc_html_e( 'Block', 'celestial-lunar-phase' ); ?></h2>
				<p><?php esc_html_e( 'Search for “Celestial Lunar Phase” in the block inserter to place the widget visually in the editor.', 'celestial-lunar-phase' ); ?></p>

				<h2><?php esc_html_e( 'Notes', 'celestial-lunar-phase' ); ?></h2>
				<ul style="list-style:disc;padding-left:20px;">
					<li><?php esc_html_e( 'This plugin uses WeatherAPI astronomy data and bundled local moon phase images.', 'celestial-lunar-phase' ); ?></li>
					<li><?php esc_html_e( 'Visitor auto-location is not enabled in this version, which avoids exposing your API key in the browser.', 'celestial-lunar-phase' ); ?></li>
					<li><?php esc_html_e( 'API responses are cached for 12 hours per location and date.', 'celestial-lunar-phase' ); ?></li>
				</ul>
			</div>
			<?php
		}
		...
		private function render_widget( $args ) {
			...
			if ( empty( $settings['api_key'] ) ) {
				return $this->render_notice( __( 'Celestial Lunar Phase is not configured yet. Please add your WeatherAPI key in Settings.', 'celestial-lunar-phase' ) );
			}

			if ( empty( $location ) ) {
				return $this->render_notice( __( 'Please set a default location in the plugin settings or pass a location in the shortcode or block.', 'celestial-lunar-phase' ) );
			}
			...
			$phase_name    = isset( $astro['moon_phase'] ) ? sanitize_text_field( $astro['moon_phase'] ) : __( 'Unknown', 'celestial-lunar-phase' );
			$moonrise      = isset( $astro['moonrise'] ) ? $this->format_time( $astro['moonrise'], $settings['time_format'] ) : __( 'N/A', 'celestial-lunar-phase' );
			$moonset       = isset( $astro['moonset'] ) ? $this->format_time( $astro['moonset'], $settings['time_format'] ) : __( 'N/A', 'celestial-lunar-phase' );
				...
				<div class="lpsw-content">
					...
					<div class="lpsw-meta">
						<div class="lpsw-meta-row">
							<span class="lpsw-label"><?php esc_html_e( 'Moonrise', 'celestial-lunar-phase' ); ?></span>
							<span class="lpsw-value"><?php echo esc_html( $moonrise ); ?></span>
						</div>
						<div class="lpsw-meta-row">
							<span class="lpsw-label"><?php esc_html_e( 'Moonset', 'celestial-lunar-phase' ); ?></span>
							<span class="lpsw-value"><?php echo esc_html( $moonset ); ?></span>
						</div>
						<?php if ( '' !== $illumination ) : ?>
							<div class="lpsw-meta-row">
								<span class="lpsw-label"><?php esc_html_e( 'Illumination', 'celestial-lunar-phase' ); ?></span>
					...
					<?php if ( $show_credit ) : ?>
						<div class="lpsw-credit">
							<?php echo wp_kses_post( __( 'Moon data by <a href="https://www.weatherapi.com/" target="_blank" rel="noopener noreferrer">WeatherAPI</a>. Phase images bundled locally.', 'celestial-lunar-phase' ) ); ?>
						</div>
					<?php endif; ?>
				</div>
			</div>
		...
		private function get_astronomy_data( $api_key, $location, $date ) {
			...
			if ( is_wp_error( $response ) ) {
				return new WP_Error( 'lpsw_request_failed', __( 'Unable to connect to the lunar data service right now.', 'celestial-lunar-phase' ) );
			}
			...
			if ( 200 !== $status ) {
				$message = __( 'The lunar data service returned an error.', 'celestial-lunar-phase' );
			...
			if ( ! is_array( $data ) || empty( $data['astronomy']['astro'] ) ) {
				return new WP_Error( 'lpsw_invalid_response', __( 'The lunar data response was incomplete.', 'celestial-lunar-phase' ) );
			}
